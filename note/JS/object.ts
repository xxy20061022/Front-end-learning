let k: object;  // k的值可以是任意（非原始类型）的值，包含：对象、函数、数组等
...
let l: Object;  // k的值可以是任意（非原始类型）的值，包含：对象、函数、数组等
l = { name: '张三' };
l = [1, 3, 5, 7, 9];
l = function () { };
l = new String('123');
...
let person4: { name: string; age: number; gender: string };
...
let person: {
    name: string;
    age: number;
    [key: string]: any;  // 索引签名，完全可以不使用 key 这个单词，换成其他的也可以
};
